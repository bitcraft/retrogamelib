    ...
    def __init__(self):
        ...
        self.head_pics = [pygame.image.load(os.path.join("data", x))
                          for x in ["head-normal.png", "head-upright.png",
                                    "head-downright.png"]]
        self.normal_leg_pic = pygame.image.load(
            os.path.join("data", "legs-normal.png"))
            ...
            if self.rect.colliderect(platform.rect):
                if self.jump_speed >= 0:
                    if platform.is_slope:
                        m = sin(radians(platform.slope)) / \
                            cos(radians(platform.slope))
        ...
        if slope != 0:
            ...
            if slope < 0:
                ...
                self.image = pygame.image.load(
                    "data/platform-shallow-right.png")
                self.slope_offset = -5
            else:
        ...
        if aliencount < 2:
            if random.randint(0, 100) > 95:
                if random.randint(0, 1) == 1:
                    alien = Alien()
                    alien.rect.topleft = [
                        258 - camera.offset[0], random.randint(0, 200)]
                    alien.direction.x = -1
                else:
                    alien = Alien()
                    alien.rect.topleft = [-5 -
                                          camera.offset[0], random.randint(0, 200)]
                    alien.direction.x = 1
